...
from unicore.data import (AppendTokenDataset, Dictionary, EpochShuffleDataset,
                          ...
                          RightPadDataset, RightPadDataset2D, SortDataset, data_utils)
...
class ContrasRankTest(UnicoreTask):
    ...
    def load_mols_dataset(self, data_path, atoms, coords, **kwargs):
        #print(self.dictionary.__dict__)
        dataset = LMDBDataset(data_path)
    ...
    def load_pockets_dataset(self, data_path, **kwargs):
        #print(self.pocket_dictionary.__dict__)
        dataset = LMDBDataset(data_path)
        ...
        src_pocket_dataset = TokenizeDataset(
            src_pocket_dataset,
            self.pocket_dictionary,
            max_seq_len=self.args.pocket.max_seq_len,
        )
        coord_pocket_dataset = KeyDataset(apo_dataset, "pocket_coordinates")
        src_pocket_dataset = PrependAndAppend(
            src_pocket_dataset,
    ...
    def test_pcba_target(self, name, model, seq, **kwargs):
        ...
        mol_data = torch.utils.data.DataLoader(mol_dataset, batch_size=bsz, num_workers=0,
                                               collate_fn=mol_dataset.collater, shuffle=False)
        ...
        pocket_data = torch.utils.data.DataLoader(pocket_dataset, batch_size=bsz, collate_fn=pocket_dataset.collater, shuffle=False)
        pocket_reps = []
        pocket_names = []
        ...
        with open(f'{self.args.results_path}/PCBA/{name}/smiles.smi', 'w') as fw:
            fw.write('\n'.join(mol_names))

        res = pocket_reps @ mol_reps.T
        #np.save(f"./{name}_saved_res.npy", res)
    ...
    def test_pcba(self, model, **kwargs):
        header = ['ckpt', 'AUC', 'BEDROC', 'EF-0.005', 'EF-0.01', 'EF-0.02', 'EF-0.05', 'RE-0.005', 'RE-0.01', 'RE-0.02', 'RE-0.05']
        results = []
        save_name = f"{os.path.split(model.args.path)[0]}/PCBA_test_results.csv"
        ckpt_path = os.path.abspath(model.args.path)
        if os.path.exists(save_name):
            with open(save_name, 'r') as f:
                tested_ckpts = {
                    line.strip().split(',')[0]:dict(zip(header[1:], line.strip().split(',')[1:])) \
                        for line in f.readlines()
                    }
            if ckpt_path in tested_ckpts:
                print(f"# PCBA test results of {model.args.path} already exist.")
                for k,v in tested_ckpts[ckpt_path].items():
                    print(f'{k} {v}')
                return
            else:
        ...
        results.append(f'{round(np.mean(auc_list) * 100, 3)}')
        results.append(f'{round(np.mean(bedroc_list) * 100, 3)}')
        for key in ef_list:
            ...
            results.append(f'{round(np.mean(ef_list[key]), 3)}')
        for key in re_list:
            ...
            results.append(f'{round(np.mean(re_list[key]), 3)}')
        with open(save_name, "a") as f:
    ...
    def test_dude_target(self, target, model, seq, **kwargs):
        ...
        #data_path = f"{self.args.data}/DUD-E/raw/all/" + target + "/mols_real.lmdb"
        ...
        with open(f"{self.args.results_path}/DUDE/{target}/smiles.smi", 'w') as fw:
            fw.write('\n'.join(mol_names))
        auc, bedroc, ef_list, re_list = cal_metrics(labels, res_single, 80.5)
    ...
    def test_dude(self, model, **kwargs):
        header = ['ckpt', 'AUC', 'BEDROC', 'EF-0.005', 'EF-0.01', 'EF-0.02', 'EF-0.05', 'RE-0.005', 'RE-0.01', 'RE-0.02', 'RE-0.05']
        results = []
        save_name = f"{os.path.split(model.args.path)[0]}/DUDE_test_results.csv"
        ckpt_path = os.path.abspath(model.args.path)
        if os.path.exists(save_name):
            with open(save_name, 'r') as f:
                tested_ckpts = {
            ...
            if ckpt_path in tested_ckpts:
                print(f"# DUDE test results of {model.args.path} already exist.")
                for k,v in tested_ckpts[ckpt_path].items():
                    print(f'{k} {v}')
                return
            else:
