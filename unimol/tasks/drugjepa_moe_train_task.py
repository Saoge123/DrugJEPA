...
def co_upcycling_init(model, ckpt_path):
    ckpt_state_dict = torch.load(ckpt_path)['model']
    state_dict = model.state_dict()
    state_dict_upcycling = {}
    for p_name, p in state_dict.items():
        if 'moe_block.moe.experts.experts' in p_name:
            name_items = p_name.split('.')
            idx_layer = int(p_name.split('.')[2])
            idx_ffn_layer = int(name_items[-2])
            if name_items[-1] == 'weight':
                idx_ffn_layer_ = idx_ffn_layer if idx_ffn_layer > 0 else 1
                p_name_ = f'encoder.layers.{idx_layer}.fc{idx_ffn_layer_}.weight'
                if p_name_ not in ckpt_state_dict:
                    p_name_ = f'encoder.layers.{idx_layer}.fc{idx_ffn_layer_-1}.weight'
                    weight = ckpt_state_dict[p_name_]
                    #std = weight.std()
                    #noise = torch.randn_like(weight) * (std * 0.015)
                    #weight.add_(noise)
                    #print(f"Added noise to {p_name_}, weight change L2: {noise.norm().item():.6f}")
                else:
                    weight = ckpt_state_dict[p_name_]
                state_dict_upcycling[p_name] = weight

            elif name_items[-1] == 'bias':
                idx_ffn_layer_ = idx_ffn_layer if idx_ffn_layer > 0 else 1
                p_name_ = f'encoder.layers.{idx_layer}.fc{idx_ffn_layer_}.bias'
                if p_name_ not in ckpt_state_dict:
                    p_name_ = f'encoder.layers.{idx_layer}.fc{idx_ffn_layer_-1}.bias'
                state_dict_upcycling[p_name] = ckpt_state_dict[p_name_]
        elif p_name in ckpt_state_dict:
            state_dict_upcycling[p_name] = ckpt_state_dict[p_name]

    if len(state_dict_upcycling) == 0:
        raise ValueError('No parameter for restoring ...')
    return state_dict_upcycling


@register_task("train_drugjepa_moe")
class Train_DrugJEPAMoE(UnicoreTask):
    ...
    def load_dataset(self, split, **kwargs):
        ...
        if split == "train" or (split == "valid" and self.args.valid_set == "TIME"):
            ...
            with open(f'{self.args.data}/sequence_embeddings.pkl', 'rb') as frb:
                sequence_dataset = pickle.load(frb)
            ...
            """print('##### save final pair label for training #####')
            with open('./training_data.json', 'w', encoding='utf-8') as f:
                json.dump(pair_label, f, ensure_ascii=False, indent=4)"""
            
            pair_dataset = PairDataset(self.args, pocket_dataset, mol_dataset, pair_label, sequence_dataset=sequence_dataset, split=split, use_cache=True, cache_dir=self.args.data)
            ...
            pair_dataset = PairDataset(self.args, pocket_dataset, mol_dataset, pair_label, split, sequence_dataset=sequence_dataset, use_cache=False)
    ...
    def build_model(self, args):
        ...
        if args.finetune_mol_model is not None:
            print(f"Upcycling mol model from pretrained model weight {args.finetune_mol_model}")
            #print(model.mol_model)
            if os.path.exists(args.finetune_mol_model):
                mol_state_dict_upcycling = co_upcycling_init(model.mol_model, args.finetune_mol_model)
            else:
                print(f"Pretrained mol model weight {args.finetune_mol_model} not found, skip upcycling")
            """for k in mol_state_dict_upcycling:
                mol_state_dict_upcycling[k] = mol_state_dict_upcycling[k].to(device)"""
            model.mol_model.load_state_dict(mol_state_dict_upcycling, strict=False)

        if args.finetune_pocket_model is not None:
            print(f"Upcycling pocket model from pretrained model weight {args.finetune_pocket_model}")
            pkt_state_dict_upcycling = co_upcycling_init(model.pocket_model, args.finetune_pocket_model)
            """for k in pkt_state_dict_upcycling:
                pkt_state_dict_upcycling[k] = pkt_state_dict_upcycling[k].to(device)"""
            model.pocket_model.load_state_dict(pkt_state_dict_upcycling, strict=False)
    ...
    def train_step(
        ...
        net_output = model(
            ...
            **sample["lig"]["net_input"],
            seq_emb=sample["sequence_emb"],
            protein_sequences=sample["protein"],
            features_only=True,
            fix_encoder=False,
            is_train=loss.training
        )

        model.train()
        model.set_num_updates(update_num)
        with torch.autograd.profiler.record_function("forward"):
            loss, sample_size, logging_output = loss(net_output, sample, model.logit_scale)
            #print(loss)
        """for name, p in model.named_parameters():
            if p.device != loss.device:
                print(name, p.device, loss.device)"""
