...
class DistanceDataset(BaseWrapperDataset):
    def __init__(self, dataset, max_seq_len=128):
    ...
    def __getitem__(self, idx):
        coords = self.dataset[idx].numpy()
        #if coords.shape[1] > self.max_seq_len:
        #    return None
        dist = [
            torch.from_numpy(distance_matrix(pos, pos).astype(np.float32)) for pos in coords
            ]
        return dist
