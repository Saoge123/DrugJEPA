...
class Experts(Module):
    ...
    def all_experts_to_cpu_besides(self, selection):
        """
        来将未使用的专家迁移到 CPU,可节约显存,但可能导致：

                1. 某些专家的参数仍在 CPU 上；

                2. 而 DDP 要求所有参数必须在当前 GPU 上；
...
class MoE(Module):
    ...
    def __init__(self,
        ...
        differentiable_topk_fused = True,
        is_distributed = None,
        allow_var_seq_len = True,  # default is False
        expert_dim_hidden = None,
        expert_dropout = 0.0,
    ):
        """
        allow_var_seq_len: 
            False (默认): Experts模块默认要求每个专家接收到的token数量必须一致, 否则触发断言。
            True: 允许每个专家接收不同数量的token, 并自动进行padding和对齐, 这在某些情况下可能是必要的, 例如DDP。
        如果调用了all_experts_to_cpu_besides(), 它会把未使用的专家迁移到CPU, 这在某种程度上“绕过”了后续的分布式逻辑。如果不使用all_experts_to_cpu_besides, 所有专家都留在GPU, 分布式逻辑开始生效, 会触发了token数量不一致的问题。
        """
        super().__init__()
        self.dim = dim
        ...
        experts = default(experts, lambda: [Expert(dim=dim, dim_hidden=expert_dim_hidden, dropout=expert_dropout) for _ in range(num_experts)])
    ...
    def forward(
        ...
        # dispatch
        expert_inputs = einsum('b n d, b n e c -> b e c d', x, dispatch_tensor.to(x))
        #expert_inputs = einsum('b n d, b n e c -> b e c d', x.to(dispatch_tensor), dispatch_tensor)
        ...
        output = einsum('b e c d, b n e c -> b n d', expert_outputs, combine_tensor.to(expert_outputs))
        #output = einsum('b e c d, b n e c -> b n d', expert_outputs.to(combine_tensor), combine_tensor)
