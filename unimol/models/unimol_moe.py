...
from .transformer_moe_encoder_with_pair import TransformerMoEEncoderWithPair
...
@register_model("unimol_moe")
class UniMolModelMoE(BaseUnicoreModel):
    @staticmethod
    def add_args(parser):
        ...
        parser.add_argument(
            ...
            type=float,
            default=0.2,
            help="threshold for training in MoE",
        )
        parser.add_argument(
            "--threshold-eval",
            type=float,
            default=0.2,
            help="threshold for evaluation in MoE",
        )
        parser.add_argument(
            "--capacity-factor-train",
            type=float,
            default=1.25,
            help="capacity factor for training in MoE",
        )
        parser.add_argument(
            "--capacity-factor-eval",
            type=float,
            default=2.0,
            help="capacity factor for evaluation in MoE",
        )
        parser.add_argument(
            "--balance-loss-coef",
            type=float,
            default=1e-2,
            help="balance loss coefficient in MoE",
        )
        parser.add_argument(
            "--router-z-loss-coef",
            type=float,
            default=1e-3,
            help="router z loss coefficient in MoE",
        )
        parser.add_argument(
            "--add-ff-before",
            type=bool,
            default=True,
            help="add feedforward layer before MoE in MoE",
        )
        parser.add_argument(
            "--add-ff-after",
            type=bool,
            default=True,
            help="add feedforward layer after MoE in MoE",
        )
        parser.add_argument(
            "--expert-dropout",
            type=float,
            default=0.0,
            help="dropout probability in expert layer in MoE",
        )
        parser.add_argument(
            "--allow-var-seq-len",
            type=Boolean,
    ...
    def __init__(self, args, dictionary):
        super().__init__()
        unimol_moe_base_architecture(args)
        self.args = args
        #print("6666", dictionary.bos(), dictionary.eos(), dictionary.pad())
        ...
        self.encoder = TransformerMoEEncoderWithPair(
            ...
            add_ff_before=args.add_ff_before,
            add_ff_after=args.add_ff_after,
...
@register_model_architecture("unimol_moe", "unimol_moe")
def moe_base_architecture(args):
    ...
    #args.moe_arch = getattr(args, "moe_arch", 'alternating')
    args.num_experts = getattr(args, "num_experts", 8)
    args.gating_top_n = getattr(args, "gating_top_n", 2)
    args.threshold_train = getattr(args, "threshold_train", 0.2)
    args.threshold_eval = getattr(args, "threshold_eval", 0.2)
    args.capacity_factor_train = getattr(args, "capacity_factor_train", 1.25)
    args.capacity_factor_eval = getattr(args, "capacity_factor_eval", 2.0)
    args.balance_loss_coef = getattr(args, "balance_loss_coef", 1e-2)
    args.router_z_loss_coef = getattr(args, "router_z_loss_coef", 1e-3)
    args.add_ff_before = getattr(args, "add_ff_before", True)
    args.add_ff_after = getattr(args, "add_ff_after", True)
    args.expert_dropout = getattr(args, "expert_dropout", 0.0)
    args.allow_var_seq_len = getattr(args, "allow_var_seq_len", True)

@register_model_architecture("unimol_moe", "unimol_moe_base")
def unimol_moe_base_architecture(args):
